...
def fit_profile(hnum,res,ver,dmo,snum,test_plot,vcirc_plot):
	grain = 60
	if dmo == 0: ###For hydro runs ###
		...
		if res == '_14':
			...
			fix = 1
		dm = hdf['particles/dm']['mass'].as_matrix()*fix
		...
		dsp = np.vstack((dp,sp))
		dsm = np.append(dm,sm)
		dsc = mikecm(fname = dsp,nofile=True,pmass=dsm)#np.array([np.float(hdf['props']['halox'])*1e3,np.float(hdf['props']['haloy'])*1e3,np.float(hdf['props']['haloz'])*1e3])#
		...
		totden = (massall+smassall)/(4*3.14159*x**3)/dlogr
...
#				bestfit = fit
                ...
                test_plot.add_line(x,totden,res)
                test_plot.add_dmoline(x,totden,res)
		totmass = massall+smassall
		vcirc_plot.add_line(x,totmass,res)
		if res =='_13':
			test_plot.fit_einasto()
		...
		dpos = hdf['particles/dm']['r'].as_matrix()*1000/(red+1)#np.sqrt((dp[:,0]-dsc[0])**2+(dp[:,1]-dsc[1])**2+(dp[:,2]-dsc[2])**2)/.71*1000/(red+1)
	...
	#			(bestfit2, cmatrix)= opt.curve_fit(nfwcored,x[fit_lims],np.log(totden[fit_lims]),p0 = (n,rc),bounds=param_bounds)
               ...
               #                         chimin = chisq
               #                         bestfit2 = fit
                vcirc_plot.add_line(bin_edge[:grain-1],massall,res)
		#test_plot = radpro()
		#test_plot.add_line(x,totden,res)
...
ver = '5_12_16'
dmo = 0
time, coresize = fit_profile(hnum,res,ver,dmo,172,test_plot,vcirc_plot)
res = '_13'
time, coresize = fit_profile(hnum,res,ver,dmo,172,test_plot,vcirc_plot)
res = ''
time, coresize = fit_profile(hnum,res,ver,dmo,172,test_plot,vcirc_plot)

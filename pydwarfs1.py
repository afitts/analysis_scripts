...
def nfw(r,mvir,c):
    ...
    rvir = (3/4.*mvir*(1/(np.pi*overden*rhocrit)))**(1/3.)
    rs = rvir/c
    #return mvir*gc*(np.log(1+r/rs)-r/rs*(1+r/rs)**-1)
    rho0 = rhocrit*overden*c**3*gc/3
...
def radpro_df(pathname,hnum,res,ver,hist,dmo,rhalf,rmax,i,grain):
  massp = 1.67262178e-24 #in grams
  gamma = 5./3.
  kb = 1.3806e-26 #in km^2*g/s^2 
  G = 4.3e-6 #in kpc/M_sun (km/s)^2   
  ...
  if dmo == 0:
    ...
    hdf.close()
    return massall, gmassall, smassall, x, rvir,rhalf,rmax, temp, den, sfr,red,dlogr, tempall,tbin[1:],cumutempall,cNFW,vmax,fullmetal,metal,dmrhalf,gmrhalf,smrhalf,dm500pc,gm500pc,sm500pc,sum(dm),sum(gm),sum(smm),vmaxx,vgmax,rout
  else:
    w = 0
    while sortdmp[w] <=.5:
      w+=1
    dm500pc=sum(sortdm[:w])
    hdf.close()
    return massall, x,cNFW,vmax,dm500pc
...
class trip_correl(object):
	...
	def __init__(self,sm):
		...
		self.trip2.set_xlabel('$M_\star$',fontsize=16)
		self.trip1.set_ylabel(r'$r_\mathrm{1/2}$ $(\mathrm{kpc})$',fontsize=16)
		self.trip2.set_ylabel(r'$M_{\mathrm{dyn}}(<r_{1/2})$',fontsize=16)
	...
	def add_point(self,hnum,x,dmox,rhalf,metal,totrhalf,smrhalf,smass,res,hismass=0,hirhalf=0,totmass=0,dmomass=0):#tot500=0,dmo500=0):
		...
		if hnum == '10v':
			...
			isodwarf = isodwarf[:,1:10]
			satmwdwarf = np.genfromtxt('satMWDwarfs.dat')
			satmwdwarf = satmwdwarf[:,1:10]
			self.trip1.errorbar(isodwarf[:,6],4/3.*isodwarf[:,0]/1000,yerr=[isodwarf[:,2]/1000,isodwarf[:,1]/1000],xerr=[isodwarf[:,8],isodwarf[:,7]],color='k',label='%s'%hnum, fmt='o',markersize=5,elinewidth=2)
			self.trip1.errorbar(satmwdwarf[:,6],4/3.*satmwdwarf[:,0]/1000,yerr=[satmwdwarf[:,2]/1000,satmwdwarf[:,1]/1000],xerr=[satmwdwarf[:,8],satmwdwarf[:,7]],color='0.6',label='%s'%hnum, fmt='o',markersize=5,elinewidth=2)
			# vv for mdyn/mstar vv FIX Y ERROR PLEAAAAASE JGIREOQHJGKDLAGJDKSAL;FDJASK
			yerp = np.sqrt((isodwarf[:,5]/isodwarf[:,3])**2+(isodwarf[:,8]/isodwarf[:,6])**2)*isodwarf[:,3]/(np.log10((10**isodwarf[:,6])/2))
			yerm = np.sqrt((isodwarf[:,4]/isodwarf[:,3])**2+(isodwarf[:,7]/isodwarf[:,6])**2)*isodwarf[:,3]/(np.log10((10**isodwarf[:,6])/2))
			self.trip2.errorbar(isodwarf[:,6],np.log10(10**(isodwarf[:,3])/((10**isodwarf[:,6])/2)),yerr=[yerp,yerm],xerr=[isodwarf[:,8],isodwarf[:,7]],color='k',label='%s'%hnum, fmt='o',markersize=5,elinewidth=2)
			yerp = np.sqrt((satmwdwarf[:,5]/satmwdwarf[:,3])**2+(satmwdwarf[:,8]/satmwdwarf[:,6])**2)*satmwdwarf[:,3]/(np.log10((10**satmwdwarf[:,6])/2))
			yerm = np.sqrt((satmwdwarf[:,4]/satmwdwarf[:,3])**2+(satmwdwarf[:,7]/satmwdwarf[:,6])**2)*satmwdwarf[:,3]/(np.log10((10**satmwdwarf[:,6])/2))
			self.trip2.errorbar(satmwdwarf[:,6],np.log10(10**(satmwdwarf[:,3])/((10**satmwdwarf[:,6])/2)),yerr=[yerp,yerm],xerr=[satmwdwarf[:,8],satmwdwarf[:,7]],color='0.6',label='%s'%hnum, fmt='o',markersize=5,elinewidth=2)
			## vv for mdyn vv ##
...
class radpro(object):
	...
	def __init__(self,sm):
		...
		self.sub.set_xlim(.1,10)#(.06,60)
		self.sub.set_ylim(1e5,1e9)
		pylab.rcParams['xtick.major.pad']='6'
		pylab.rcParams['ytick.major.pad']='6'
  
	def add_line(self,x,rhalf,totmass,dlogr,res,red):
		if res == '':
			clr = 'r'
			lbl = 'LOW '
		else:
			clr = 'k'
			lbl = 'HI '
		self.sub.loglog(x,totmass/(4*3.14159*x**3)/dlogr, color = '%s'%clr,linewidth=4,label = '%sHYDRO'%lbl)
		np.savetxt('Halo%s_radden_hydro.out'%(hnum),np.column_stack((x,totmass/(4*3.14159*x**3)/dlogr)))
		...
		self.sub.axvline(x=rhalf,color='r',linestyle='--',linewidth=1)

	def add_dmoline(self,dmox,dmomass,dlogr,res,c,hnum):
		if res == '':
			clr = 'r'
			lbl = 'LOW '
		else:
			clr = 'k'
			lbl = ''
		self.sub.loglog(dmox,dmomass/(4*3.14159*dmox**3)*0.83120300751/dlogr, color = 'lightgrey',linestyle='--',linewidth=4, label = '%sDMO'%lbl)
		np.savetxt('Halo%s_radden_dmo.out'%(hnum),np.column_stack((dmox,dmomass/(4*3.14159*dmox**3)*0.83120300751/dlogr)))
  ...
  #===========================================================================#
		chimin =10000
		jose = np.genfromtxt('f11den.txt')
param_bounds=([4e9,.5],[1.3e10,26])
(fit, cmatrix)= opt.curve_fit(nfw,dmox[(dmox>.2)],dmomass[(dmox>.2)],bounds=param_bounds)#dmox,dmomass/(4*3.14159*dmox**3),p0=(mvir,c))
print fit
		chisq = sum((dmomass[(dmox>.2) & (dmox<100)]-nfw(dmox[(dmox>.2) & (dmox<100)],*fit))**2/(nfw(dmox[(dmox>.2) & (dmox<100)],*fit)))#(dmomass/(4*3.14159*dmox**3)-nfw(dmox,*fit))**2/(dmomass/(4*3.14159*dmox**3))**2)
		print chisq,dlogr
		if chisq < chimin:
			chimin = chisq
			bestfit = fit

  		dof = len(dmomass)-len(fit)
  		print fit, chisq, dof
		self.sub.loglog(dmox,nfw(dmox,*bestfit),'g-',linewidth=2, label='nfw') #Core Fitting

		#self.sub.loglog(jose[:,0],jose[:,1],'m')


	def save(self,date,hnum,red,grain):
		self.sub.legend(loc=3,prop={'size':10})
		self.fig.savefig('radden_halo%s_%dbin_%s.pdf'%(hnum,grain,date),transparent=True)
		self.fig.show()
		#plt.close()
...
def main():
  ...
  sm = cm.ScalarMappable(cmap=my_cmap,norm=co.Normalize(vmin=5.6739, vmax=7.176))#vmin=5.874853, vmax=7.11806)) 11_13 limits
  sm._A = []
  ...
  dmover = ['11_13','11_13','11_13','11_13','11_13','11_13','5_12_16','5_12_16','11_13','11_13','11_13','11_13','11_13','11_13']
  ...
  for w in np.arange(len(hnum)):
   #for i in sp:
    ...
    if hnum[w] == '10q' or hnum[w] == '10v':
     i = 600
    try:
     dmass,gmass,smass,x,rvir,rhalf,rmax, temp, den, sfr,red,dlogr,tempall,cumutempx,cumutempy,cNFW,vmax,fullmetal,metal,dmrhalf,gmrhalf,smrhalf,dm500pc,gm500pc,sm500pc,dmm,gmm,smm,vmaxx,vgmax,rout = radpro_df(pathname,hnum[w],res[w],ver[w],hist,0,0,0,i,grain)
     totmass = dmass+gmass+smass
     hismass = smass
     hirhalf = rhalf
     #radpro_plot.add_line(x,rhalf,totmass,dlogr,res[w],red)
     if hnum[w] == '848':
       hnum[w] = '2'
     if hnum[w] == '10q' or hnum[w] == '10v':
       bob = hnum[w]
     ...
     ## hist = np.genfromtxt(pathname+'halo%smerger_hist.txt'%hnum[w])
     ## dmomass,dmox,dmocNFW,dmovmax,dmo500pc = radpro_df(pathname,hnum[w],res[w],dmover[w],hist,1,rhalf,rmax,i,grain)
     radpro_plot.add_dmoline(dmox,dmomass,dlogr,res[w],dmocNFW,hnum[w])
     ## res[w] = '_14'
     ## pathname = '/nobackup/afitts/Gadget-2.0.7/production/gizdm%s%s_raw_output/analysis/'%(hnum[w],res[w])
     ## hist = np.genfromtxt(pathname+'halo%smerger_hist.txt'%hnum[w])
     ## dmomass,dmox,dmocNFW,dmovmax,dmo500pc = radpro_df(pathname,hnum[w],res[w],dmover[w],hist,1,rhalf,rmax,i,grain)  
    ...
    #totrhalf = dmrhalf+gmrhalf+smrhalf
    #tot500pc = dm500pc+gm500pc+sm500pc
    #radpro_plot.add_line(x,rhalf,totmass,dlogr,res[w])
    #trip_correl_plot.add_point(hnum[w],x,rhalf,metal,totrhalf,smrhalf,smass,res[w],hismass,hirhalf)
    #metal_plot = metal_dist(sm)
    #metal_plot.add_dist(fullmetal,grain)
    #metal_plot.save(hnum[w],date,grain)
    ...
    #dmomass,dmox,dmocNFW,dmovmax,dmo500pc = radpro_df(pathname,hnum[w],res[w],dmover[w],hist,1,rhalf,rmax,i,grain)
    #radpro_plot.add_dmoline(dmox,dmomass,dlogr,res[w])
    radpro_plot.save(date,hnum[w],red,47)
    #rhoratio_plot.add_line(x,dmox,totmass,smass,dmomass,rhalf)
    #rhoratiovmstar_plot.add_point(hnum[w],x,dmox,tot500pc,dmo500pc,smass,dlogr,res[w])
    #print hnum[w]

...
def radpro_df(pathname,hnum,res,ver,hist,dmo,rhalf,rmax,i,grain,dmover,dmorvir=0):
  massp = 1.67262178e-24 #in grams
  gamma = 5./3.
  kb = 1.3806e-26 #in km^2*g/s^2 
  G = 4.3e-6 #in kpc/M_sun (km/s)^2   
  ...
  if dmo == 0:
    binz = np.logspace(np.log(.0014),np.log(dmorvir),48,base=np.e)
  ...
  cNFW = 0#np.float(hdf['props']['cNFW'])
  ...
  if dmo == 0:
    ...
    den = hdf['particles/gas']['rho'].as_matrix()*1e10*1.99e33/1.67e-24/(3.806e21)**3
    ...
    svx = hdf['particles/star']['vx'].as_matrix()*1000
    svy = hdf['particles/star']['vy'].as_matrix()*1000
    svz = hdf['particles/star']['vz'].as_matrix()*1000
    vdisp = np.sqrt(np.std(svx)**2+np.std(svy)**2+np.std(svz)**2)/np.sqrt(3.)
    smcentral = 0
    if hnum != '1084':
     w = 0
     while sortsp[w] <=0.1*rvir:
       w+=1
     smcentral=sum(sortsm[:w])
     w = 0
     while sortsmcumu[w] <= 0.5*smcentral:
       rhalf = sortsp[w]
       w+=1
    else:
     rhalf =0
    ...
    w = 0
    vdisp_rhalf = np.sqrt(np.std(svx[spos<rhalf])**2+np.std(svy[spos<rhalf])**2+np.std(svz[spos<rhalf])**2)/np.sqrt(3.)    

    hdf.close()
    cgas = sum(gmass[temp<=1e4])
    cgasrhalf = sum(gmass[(temp<=1e4) & (gpos<rhalf)])
    return massall, gmassall, smassall, x, rvir,rhalf,rmax, temp, den, sfr,red, tempall,tbin[1:],cumutempall,cNFW,vmax,fullmetal,metal,dmrhalf,gmrhalf,smrhalf,dm500pc,gm500pc,sm500pc,sum(dm),sum(gm),smcentral,vmaxx,vgmax,rout,dmnum,gnum,snum,cgas,cgasrhalf,vdisp,vdisp_rhalf
  else:
    w = 0
    power = 1
    ...
    dm500pc=sum(sortdm[:w])
    hdf.close()
    return massall, x,cNFW,vmax,dlogr,dm500pc,dmnum,prad,rvir 
...
class plot_phase(object):
	...
	def add_plot(self,hymass,hygmass,hysmass,temp,den,sfr,red,grain):
		#global count,grain
		...
		self.sub.text(0.25, 0.2,'$\mathrm{M_{baryon}}$ = %.3e'%sum(hysmass+hygmass), ha='center', va='center', transform=self.sub.transAxes, fontsize = 15)
		...
		#plt.show()
		#plt.close()
	def save(self,hnum,snum,date,grain):
		self.fig.savefig('phase_halo%s_%dbin_%03d.pdf'%(hnum,grain,snum),Transparent=True)
		self.fig.show()
...
class rhoratiovr(object):
	...
	def __init__(self,sm):
		...
		self.sub.set_xlabel(r'$\mathrm{r}/\mathrm{r}_{1/2}$',fontsize=16, labelpad=-10)#'r/R$_{vir}$'
	...
	def add_line(self,x,dmox,totmass,smass,dmomass,rhalf,prad):
		...
		self.sub.loglog(x[x<prad]/rhalf,(totmass[x<prad])/(4*3.14159*(x[x<prad]/rhalf)**3)/(dmomass[x<prad]*0.83120300751/(4*3.14159*(dmox[x<prad]/rhalf)**3)), linestyle='--',color=self.sm.to_rgba(np.log10(sum(smass))),linewidth=4)
		self.sub.loglog(x[x>prad]/rhalf,(totmass[x>prad])/(4*3.14159*(x[x>prad]/rhalf)**3)/(dmomass[x>prad]*0.83120300751/(4*3.14159*(dmox[x>prad]/rhalf)**3)), color=self.sm.to_rgba(np.log10(sum(smass))),linewidth=4)  
...
class rhalfvmstar(object):
	...
	def __init__(self,sm=0):
		...
		self.sub.set_xlim(.3,1.5)#2e-1,2e0)
		self.sub.set_ylim(5.5,7.1)#4e5,2e7)
	...
	def add_point(self,hnum,rhalf,smass,clr):
		if clr == 'k':
			self.sub.scatter(rhalf,np.log10(smass),s=90,color = clr,label='CDM')
		else:
			self.sub.scatter(rhalf,np.log10(smass),s=90,color = clr)
	def add_sipoint(self,hnum,rhalf,smass,clr):
		if clr == 'k':
			self.sub.scatter(rhalf,np.log10(smass),s=90,color = clr,marker = 's',label='SIDM')
		else:
			self.sub.scatter(rhalf,np.log10(smass),s=90,color = clr,marker = 's',)
...
class rhoratio(object):
	...
	def __init__(self,sm,ver):
		...
		if ver == 0:
			...
			self.sub.set_xlabel(r'$\mathrm{GoodnessOfFit}$',fontsize=20)
			self.sub.set_xlim(1e-2,6e-2)
	...
	def add_point(self,hnum,x,dmox,totmass,dmomass,smass,dlogr,res,rhalf=0,q = 0):
		if res == '':
			mkr = '^'
		else:
			mkr = 'o'
		...
		if self.ver == 0:
			...
			qsq = np.genfromtxt('hydro_dmo_fits.out')
			self.sub.scatter(qsq[q,1],(totmass[v]/(4*3.14159*(x[v])**3)/dlogr)/((dmomass[w]*0.83120300751/(4*3.14159*(dmox[w])**3)/dlogr)),marker = '%s'%mkr,s=80,color=self.sm.to_rgba(np.log10(sum(smass))),label='%s'%hnum)#(80*sum(hysmass)/5e6)
	...
	def save(self,date):
		if self.ver == 0:
		  ...
		  self.fig.savefig('rhohydrorhodmo_500pc_vs_GoF_loglog_%s.pdf'%date,transparent=True)
		self.fig.show()
		#plt.close()
...
class alpha_vs_mstarmhalo(object):
	...
	def add_point(self,hnum,totmass,smass,dmomass,rvir):
		hyd = np.genfromtxt('Halo%s_raddenZ12_hydro.out'%hnum)
		dmo = np.genfromtxt('Halo%s_raddenZ12_dmo.out'%hnum)
		den1 = hyd[:,1]
		dendmo1 = dmo[:,1]
		x1 = hyd[:,0]
		den = den1[(x1>0.01*rvir) & (x1<0.02*rvir)]
		x = x1[(x1>0.01*rvir) & (x1<0.02*rvir)]
		alpha,intercept, r_value, p_value, std_err = stats.linregress(np.log(x),np.log(den))
		self.sub.scatter(np.log10(smass/totmass),alpha,linewidth = 2,color=self.sm.to_rgba(np.log10(smass)),label='%s'%hnum)
		dendmo = dendmo1[(x1>0.01*rvir) & (x1<0.02*rvir)]
		alphadmo,intercept, r_value, p_value, std_err = stats.linregress(np.log(x),np.log(dendmo))
		

		binz = np.logspace(np.log(.0014),np.log(rvir),48,base=np.e)
		alpha_list = np.zeros(16)
		alphadmo_list = np.zeros(16)
		x_list = np.zeros(16)
		for i in np.arange(16):
			x = x1[(x1>binz[3*i]) & (x1<binz[3*i+2])]
			den = den1[(x1>binz[3*i]) & (x1<binz[3*i+2])]
			dendmo = dendmo1[(x1>binz[3*i]) & (x1<binz[3*i+2])]
			alpha_list[i],intercept, r_value, p_value, std_err = stats.linregress(np.log(x),np.log(den))
			alphadmo_list[i],intercept, r_value, p_value, std_err = stats.linregress(np.log(x),np.log(dendmo))
			x_list[i] = binz[3*i+1]
		fig = plt.figure()
		bob = fig.add_subplot(111)
		bob.semilogx(x_list,alpha_list,'k',label='HYDRO')
		bob.semilogx(x_list,alphadmo_list,'r--',label='DMO')
		bob.axvline(0.015*rvir,color = 'b',linestyle=':',label=r'1.5% R$_\mathrm{200,crit}$')
		bob.set_ylabel(r'$\alpha$')
		bob.set_xlabel(r'Radius (kpc)')
		bob.set_xlim(6e-2,6e1)
		bob.set_ylim(-5,1)
		bob.legend(loc=0)
		fig.savefig('alpha_vs_r_halo%s_lowres.pdf'%hnum,transparent=True)
		#fig = plt.figure()
		#bob = fig.add_subplot(111)
		...
		#plt.show()
		return alphadmo,alpha,np.log10(smass/totmass)

	def save(self,date):
		self.fig.savefig('alpha_vs_mstarmhalo_%s.pdf'%date,transparent=True)
		self.fig.show()

class stellarextent(object):
	"""Extenet of Stellar Mass """
	def __init__(self,sm):
		self.fig = plt.figure()
		self.sub = self.fig.add_subplot(111)
		self.sm = sm
		...
		self.sub.xaxis.set_label_coords(.48,-.07)
		self.sub.set_xlabel(r'$\rm Radius$ $(\rm kpc)$',fontsize=20, labelpad=-10)#'r/R$_{vir}$'
		self.sub.set_ylabel(r'$M_\star\:(>\mathrm{r})/M_{\star,\rm vir}$',fontsize=20, labelpad=5)
		self.sub.set_xlim(0,10)#.1,10)#(.06,60)
		self.sub.set_ylim(-2,np.log10(1.05))#1e5,1e9)
		#self.sub.grid(True,which = 'both')
		self.sub.axhline(np.log10(0.5),linestyle='--',color='k',linewidth=1)
		self.sub.annotate(r'$50\%$', xy=(9, np.log10(0.52)), xytext=(8.8, np.log10(0.52)))
		self.sub.axhline(np.log10(0.25),linestyle='--',color='k',linewidth=1)
		self.sub.annotate(r'$75\%$', xy=(9, np.log10(0.26)), xytext=(8.8, np.log10(0.26)))
		self.sub.axhline(np.log10(0.1),linestyle='--',color='k',linewidth=1)
		self.sub.annotate(r'$90\%$', xy=(9, np.log10(0.103)), xytext=(8.8, np.log10(0.103)))
		self.sub.axhline(np.log10(0.05),linestyle='--',color='k',linewidth=1)
		self.sub.annotate(r'$95\%$', xy=(9, np.log10(0.05175)), xytext=(8.8, np.log10(0.05175)))
		pylab.rcParams['xtick.major.pad']='6'
		pylab.rcParams['ytick.major.pad']='6'
		ticks = np.linspace(-2,0,3)
		plt.yticks(ticks,['0.01','0.1','1'])#[r'$10^1$',r'$10^2$'],fontsize=10) #[r'$10^6$',r'$10^7$',r'$10^8$',r'$10^9$'],fontsize=16)#
		minor_ticks=[]
		for j in range(2,10):
		  minor_ticks.append(-2.+np.log10(j))
		for j in range(2,10):
		  minor_ticks.append(-1.+np.log10(j))
		self.sub.yaxis.set_minor_locator(FixedLocator(minor_ticks))
		minor_labels = ['','','','0.05','','','','','','','','0.5','','','','','','','','']#['','30 ','','','60 ','','','']
		#self.sub.yaxis.set_minor_formatter(FixedFormatter(minor_labels))
	def add_line(self,x,rhalf,smass,dlogr,res,red,hnum,mvir,c,prad,num):
		s_cumumass = np.cumsum(smass)
		self.sub.plot(x,np.log10((sum(smass)-s_cumumass)/sum(smass)), color = self.sm.to_rgba(np.log10(sum(smass))),linewidth=2,label = 'HYDRO')
		w = 0
		while s_cumumass[w+1]<0.95*sum(smass):
			w+=1
		v = 0
		while s_cumumass[v+1]<0.99*sum(smass):
			v+=1
		#print 'r99 is ',x[v]
		#self.sub.text(1,1e8,r'%s M$_\star=$%.2e'%(ver,sum(smassall)),fontsize=16)
		#self.sub.text(6,0.95,r'$r_{95}=$%.2f'%(x[w]),fontsize=16)
		#self.sub.axvline(x=x[w],color='b',linestyle='--',linewidth=1)
	...
	def save(self,date,hnum,grain):
		...
		self.fig.savefig('StellarExtent_halo%s_%s.pdf'%(hnum,date),transparent=True)#radden_halo%s_%dbin_%s.pdf'%(hnum,grain,date),transparent=True)
		self.fig.show()
		#plt.close()
...
class radpro(object):
	...
	def __init__(self,sm):
		...
		self.sub.set_xlim(.04,60)#.1,10)#(.06,60)
		self.sub.set_ylim(1e3,1e9)#1e5,1e9)
		pylab.rcParams['xtick.major.pad']='6'
		pylab.rcParams['ytick.major.pad']='6'
  
	def add_line(self,x,rhalf,totmass,dlogr,res,red,hnum,mvir,c,prad,num):
		if res == '':
			clr = 'r'
			lbl = 'LOW '
			np.savetxt('Halo%s_raddenZ12_hydro.out'%(hnum),np.column_stack((x,totmass/(4*3.14159*x**3)/dlogr,(num))))
		else:
			clr = 'k'
			lbl = 'HI '
			np.savetxt('Halo%s_raddenZ14_hydro.out'%(hnum),np.column_stack((x,totmass/(4*3.14159*x**3)/dlogr,(num))))
		self.sub.loglog(x,totmass/(4*3.14159*x**3)/dlogr, color = '%s'%clr,linestyle='',linewidth=4,label = 'HYDRO',marker = 's')
		#self.sub.text(1,1e8,r'%s M$_\star=$%.2e'%(ver,sum(smassall)),fontsize=16)
		...
		print 'llim is ', llim
		print 'x is ',x
		print totmass/(4*3.14159*x**3)/dlogr
		param_bounds=([0.0,0.0],[1,5])
		totden = totmass/dlogr/(4*3.14159*(x)**3)
		def CNFW(mvir,c): return lambda r,n,rc : corenfw(r,mvir,c,n,rc)
		nfwcored = CNFW(mvir,c)
		(fit, cmatrix)= opt.curve_fit(nfwcored,x[x>llim],np.log(totden[x>llim]),bounds = param_bounds)
		print 'n is ', fit[0]
		print 'core radius is ',fit[1]
		residuals = np.log10(totmass[x>llim]/(4*3.14159*x[x>llim]**3)/dlogr/np.exp(corenfw(x[x>llim],mvir,c, *fit)))
		RSS = sum(residuals**2)
		self.sub.loglog(x,np.exp(corenfw(x,mvir,c, *fit)),'m',linewidth=2, label='nfw_core') #Core Fitting
		#f.write('Halo %s n = %.2e Rcore = %f RSS = %f\n'%(hnum,fit[0],fit[1],RSS))

	def add_dmoline(self,dmox,dmomass,dlogr,res,c,hnum,prad,dmonum):
		if res == '':
			clr = 'r'
			lbl = 'Z12 '
		elif res == '_14':
			...
			clr = 'k'
			lbl = 'Z13 '
		self.sub.plot(dmox,dmomass/(4*3.14159*dmox**3)*0.83120300751/dlogr, color = 'r',linestyle='',linewidth=4, label = '%sDMO'%lbl,marker='s')#'lightgrey'
  ...
  #===========================================================================#
		...
		(fit, cmatrix)= opt.curve_fit(nfw,dmox[dmox>prad],np.log(dmoden[dmox>prad]),bounds = param_bounds)
		print fit
		#self.sub.loglog(dmox,np.exp(nfw(dmox, *fit)),'c',linewidth=2, label='nfw') #Core Fitting
	...
	def save(self,date,hnum,grain):
		self.sub.legend(loc=3,prop={'size':10})
		self.fig.savefig('radden_halo%s_%s.pdf'%(hnum,date),transparent=True)#radden_halo%s_%dbin_%s.pdf'%(hnum,grain,date),transparent=True)
		self.fig.show()
		#plt.close()
...
def main1():
	...
	for w in np.arange(len(hnum)):
		f = np.genfromtxt('m12%s_ref13.txt'%hnum[w])	
		#rmin,rmax,blank,blank,mass in bin, mass(<rmax)
		mass = f[:,4]
		cumumass = f[:,5]
		r = 10**(np.log10(f[:,1])-np.log10(f[2,1]/f[1,1])/2)
		den1 = mass/(4*np.pi*r**3)/np.log10(r[2]/r[1])
		avgden = cumumass/(4/3.*np.pi*r**3)
		loc = max(np.where(avgden>200*rhocrit)[0])
		rvir = r[loc]
		print 'RVIR is', rvir
		x1 = r 
		den = den1[(x1>0.01*rvir) & (x1<0.02*rvir)]
		x = x1[(x1>0.01*rvir) & (x1<0.02*rvir)]
		alpha[w],intercept, r_value, p_value, std_err = stats.linregress(np.log(x),np.log(den))
		plt.loglog(r,den1)
		#plt.loglog(f[:,1],f[:,2])
		f = np.genfromtxt('m12%s_ref13_dmo.txt'%hnum[w])	
		#rmin,rmax,blank,blank,mass in bin, mass(<rmax)
		mass = f[:,4]
		cumumass = f[:,5]
		r = 10**(np.log10(f[:,1])-np.log10(f[2,1]/f[1,1])/2)
		...
		den = den1[(x1>0.01*rvir) & (x1<0.02*rvir)]
		x = x1[(x1>0.01*rvir) & (x1<0.02*rvir)]
		alphadmo[w],intercept, r_value, p_value, std_err = stats.linregress(np.log(x),np.log(den))
		plt.loglog(r,den1,linestyle='--')
		#plt.loglog(f[:,1],f[:,2],linestyle='--')
	plt.xlim(6e-3,7e2)
	plt.ylim(3e2,2e11)
	plt.show()
	np.savetxt('density_profile_slopes_m12_200rhocrit.txt',np.column_stack((np.zeros(len(alpha)),alpha,alphadmo)))
...
def main(): 
  ...
  sm3._A = []
  hnum =['848']#,'32503','12596','007','848','796','20192','20910','897','948','1016','32257','10q','10v','1084']
  res = ['_14','_13','_13','_11','_13','_13','_13','_13','_13','_13','_13','_13','_11','_11','_13']
  #res = ['','','','','','','','','','','','','_11','_11','']
  ...
  dmover = ver
  #dmover = ['11_13','11_13','11_13','11_13','5_12_16','11_13','5_12_16','5_12_16','11_13','11_13','11_13','11_13','5_12_16','5_12_16','5_12_16']
  ...
  #rhoratiovgof_plot= rhoratio(sm,2)
  #stellarextent_plot = stellarextent(sm)
  ...
  alphadmo = np.zeros(len(hnum))
  alphax = np.zeros(len(hnum))
  alphay = np.zeros(len(hnum))
  f = np.array([],dtype = [('name','|S10'),('Mdyn','>f4'),('Mgas','>f4'),('Mgas_cold','>f4'),('Mstar_central','>f4'),('Mdyn_rhalf','>f4'),('Mgas_rhalf','>f4'),('Mgas_cold_rhalf','>f4'),('Mstar_rhalf','>f4'),('rhalf','>f4'),('vmax','>f4'),('Mvir_dmo','>f4'),('vmax_dmo','>f4'),('rhohydro_rhodmo_500pc','>f4'),('GoF','>f4'),('rhodmo_500pc','>f4'),('power_radius','>f4'),('c_Einasto','>f4'),('vdisp','>f4'),('vdisp_rhalf','>f4')])
  for w in np.arange(len(hnum)):
   #for i in sp:
    radpro_plot = radpro(sm)
    phase_plot = plot_phase(sm)
    try:
     rhalf = rmax = 0
...
#       i = 184
     ...
     rhocrit = 139.94 #M_sun/kpc^3
     #for alpha#pathname = '/nobackup/afitts/Gadget-2.0.7/production/mfm%s%s_giz5_12_16_raw_output/analysis/'%(hnum[w],res[w])
     #for alpha#hdf = pd.HDFStore('%sdataframes/halo%s%s_giz%s_snap%d.h5'%(pathname,hnum[w],res[w],ver[w],i))
     #for alpha#smass = hdf['particles/star']['mass'].as_matrix()
     #for alpha#dmass = hdf['particles/dm']['mass'].as_matrix()
     #for alpha#gmass = hdf['particles/gas']['mass'].as_matrix()
     #for alpha#mass = np.concatenate((smass,dmass,gmass))
     #for alpha#sr = hdf['particles/star']['r'].as_matrix()*1000
     #for alpha#dr = hdf['particles/dm']['r'].as_matrix()*1000
     #for alpha#gr = hdf['particles/gas']['r'].as_matrix()*1000
     #for alpha#r = np.concatenate((sr,dr,gr))
     #for alpha#mass = np.cumsum(mass[r.argsort()])
     #for alpha#avgden = mass/(4/3.*np.pi*np.sort(r)**3)
     #for alpha#loc = max(np.where(avgden>200*rhocrit)[0])
     #for alpha#rvir = np.sort(r)[loc]
     #for alpha#smass = sum(smass[sr<0.2*rvir])
     #for alpha#dmass = sum(dmass[dr<rvir])
     #for alpha#gmass = sum(gmass[gr<0.2*rvir])
     #for alpha#hdf.close()
     #for alpha#pathname = '/nobackup/afitts/Gadget-2.0.7/production/gizdm%s%s_raw_output/analysis/'%(hnum[w],res[w])
     #for alpha#hdf = pd.HDFStore('%sdataframes/halo%s%s_giz%s_snap%d.h5'%(pathname,hnum[w],res[w],dmover[w],i))
     #for alpha#dmomass = sum(hdf['particles/dm']['mass'].as_matrix())
     #for alpha#hdf.close()
     hist = np.genfromtxt(pathname+'halo%smerger_hist.txt'%hnum[w])
     dmomass,dmox,dmocNFW,dmovmax,dlogr,dmo500pc,dmonum,prad,dmorvir = radpro_df(pathname,hnum[w],res[w],dmover[w],hist,1,rhalf,rmax,i,grain,dmover[w])
     #usualrun# np.savetxt('Halo%s_raddenZ12_dmo.out'%(hnum[w]),np.column_stack((dmox,dmomass/(4*3.14159*dmox**3)*0.83120300751/dlogr,dmonum)))
     #!dmo500pc = dmo500pc *0.83120300751
#     if bob == '10q' or bob == '10v':
#       hnum[w] = bob
#       res[w] = '_11'
     mvir,c = radpro_plot.add_dmoline(dmox,dmomass,dlogr,res[w],dmocNFW,hnum[w],prad,dmonum)
     pathname = '/nobackup/afitts/Gadget-2.0.7/production/mfm%s%s_giz%s_raw_output/analysis/'%(hnum[w],res[w],ver[w])
     hist = np.genfromtxt(pathname+'halo%smerger_hist.txt'%hnum[w])
     dmass,gmass,smass,x,rvir,rhalf,rmax, temp, den, sfr,red,tempall,cumutempx,cumutempy,cNFW,vmax,fullmetal,metal,dmrhalf,gmrhalf,smrhalf,dm500pc,gm500pc,sm500pc,dmm,gmm,smm,vmaxx,vgmax,rout,dmnum,gnum,snum,cgas,cgasrhalf,vdisp,vdisp_rhalf = radpro_df(pathname,hnum[w],res[w],ver[w],hist,0,0,0,i,grain,dmover[w],dmorvir)
     #$ 1084 dmo profile
     ...
     #$totmass, bin_edge = np.histogram(dpos,bins=binz, weights =dmass)
     #$num, bin_edge = np.histogram(dpos,bins=binz)
     totmass = dmass+gmass+smass

     #hismass = smass
     #hirhalf = rhalf
     num = dmnum + gnum + snum
     ##np.savetxt('Halo%s_raddenZ13_hydro.out'%(hnum[w]),np.column_stack((dmox,totmass/(4*3.14159*dmox**3)/dlogr,(num))))
     v = 0
     while x[v] <=.5:
        v+=1
     j = 0
     ...
     if x[v]-.5>.5-x[v-1]:
       v-=1
     if dmox[j]-.5>.5-dmox[j-1]:
       j-=1
     rhdmo500pc = (totmass[v]/(4*3.14159*(x[v])**3)/dlogr)/(dmomass[j]*0.83120300751/(4*3.14159*(dmox[j])**3)/dlogr)
     #phase_plot.add_plot(dmass,gmass,smass,temp,den,sfr,red,grain)
     #print (hnum[w],sum(dmass)+sum(gmass)+sum(smass),sum(gmass),sum(smass),dmrhalf+gmrhalf+smrhalf,gmrhalf,smrhalf,rhalf,vmax,sum(dmomass),dmovmax,rhdmo500pc,1/48.*sum((np.log(totmass[x>prad])-np.log(dmomass[x>prad]))**2))
     f = np.append(f,np.array([(hnum[w],sum(dmass)+sum(gmass)+sum(smass),sum(gmass),cgas,smm,dmrhalf+gmrhalf+smrhalf,gmrhalf,cgasrhalf,smrhalf,rhalf,vmax,sum(dmomass),dmovmax,(totmass[v]/(4*3.14159*(x[v])**3)/dlogr)/(dmomass[j]*0.83120300751/(4*3.14159*(dmox[j])**3)/dlogr),np.sqrt(1/48.*sum((np.log(totmass[(x>prad)&(x<10)])-np.log(dmomass[(x>prad) & (x<10)]))**2)),(dmomass[j]*0.83120300751/(4*3.14159*(dmox[j])**3)/dlogr),prad,c,vdisp,vdisp_rhalf)],dtype = f.dtype))
     radpro_plot.add_line(x,rhalf,totmass,dlogr,res[w],red,hnum[w],mvir,c,prad,num)
     #stellarextent_plot.add_line(x,rhalf,smass,dlogr,res[w],red,hnum[w],mvir,c,prad,num)
     ##if hnum == '948'
     ## radpro_plot = radpro(sm)
     ...
     #for alpha# alphadmo[w],alphay[w],alphax[w] = alpha_plot.add_point(hnum[w],dmass,smass,dmomass,rvir)
     #print 'bobobo is',a
     ...
     #phase_plot.save(hnum[w],snum[w],date,grain)
    except Exception,e:
     print e, 'no gas'
    ...
    #rhoratiovgof_plot.add_point(hnum[w],x,dmox,totmass,dmomass,smass,dlogr,res[w],0,w)
...
#       res[w] = '_11'    
 ...
 #! rhoratio_plot.save(date,grain)
  #rhoratiovgof_plot.save(date)
 #! rhoratiovrhalf_plot.save(date)
  #vcirc_plot.save(date)
  #mhalomstar_plot.save(date)
  #vmaxvgmax_plot.save(date)
  #for alpha#alpha_plot.save(date)
  #print 'YELLOW',alphax,alphay,alphadmo
  #for alpha#np.savetxt('density_profile_slopes_lowres_200rhocrit.txt',np.column_stack((alphax,alphay,alphadmo)))
  #mstarvmax_plot.save(date)
  #stellarextent_plot.save(date,hnum[w],grain)

  np.savetxt('plottbl_Z13_%s.out'%date,f,fmt = ['%s','%.3e','%.3e','%.3e','%.3e','%.3e','%.3e','%.3e','%.3e','%.3f','%.2f','%.3e','%.2f','%f','%f','%f','%f','%f','%f','%f'],header = '0 Halo, 1 M_vir (total), 2 Mgas, 3 Mgas_cold (<1e4) 4 Mstar(<0.1*R_vir), 5 Mdyn(<rhalf), 6 Mgas(<rhalf), 7 Mgas_cold(<rhalf), 8 Mstar(<rhalf), 9 rhalf, 10 Vmax, 11 Mvir (DMO), 12 Vmax (DMO), 13 rho_hydro/rho_dmo(500pc,corrected), 14 Hydro-to-DMO radpro GoF (Q), 15 rho_dmo(500pc,corrected), 16 Power Radius (DMO), 17 c (Einasto), 18 1D Velocity Dispersion, 19 1D Velocity Dispersion (<rhalf)')

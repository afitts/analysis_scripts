...
def corenfw(r,mvir,c,kappa_tsf,rc):
...
class vcircvr (object):
	...
	def __init__(self,sm):
		...
		self.sub.set_xlim(0,1.1e0)
		self.sub.set_ylim(0,35)
		#cb13 = fig13.colorbar(sm)
		#cb13.set_label(r'$\mathrm{log}$ $M_\star$')

	def add_line(self,hnum,x,totmass,res):
		G = 4.3e-6 #in kpc/M_sun (km/s)^2 
		v = 0
		if res == '':
			clr = 'c'
		...
		self.sub.plot(x,np.sqrt(G*cmass/x),linewidth = lw,color='%s'%clr,label='%s'%lbl)


	def save(self,date,hnum):
		self.fig.savefig('vcirc%s_profile_dmotest_%s.pdf'%(hnum,date),transparent=True)
		self.fig.show()
...
class radpro(object):
	...
	def __init__(self,sm):
		...
		self.sub.set_xlim(.02,10)#.1,10)#(.06,60)
		self.sub.set_ylim(1e5,2e9)#1e5,1e9)
		pylab.rcParams['xtick.major.pad']='6'
		pylab.rcParams['ytick.major.pad']='6'
  
	def add_line(self,x,rhalf,totmass,dlogr,res,red,hnum,mvir,c,prad):
		if res == '':
			clr = 'r'
			lbl = 'LOW '
		...
		llim = .2
		param_bounds=([0.0,0.0],[np.inf,5])
		totden = totmass#!/dlogr/(4*3.14159*(x)**3)
		def CNFW(mvir,c): return lambda r,kappa_tsf,rc : corenfw(r,mvir,c,kappa_tsf,rc)
	...
	def add_dmoline(self,dmox,dmomass,dlogr,res,c,hnum,prad,dmonum):
		if res == '':
			...
			lbl = 'Z13 '
			lw = 3.5
		self.sub.loglog(dmox,dmomass/(4*3.14159*dmox**3)*0.83120300751/dlogr, color = '%s'%clr,linewidth=lw, label = '%sDMO'%lbl)#'lightgrey'
		#self.sub.fill_between(np.linspace(1e-3,prad),1e3,1e11,facecolor='k',alpha=0.35)
		if res == '':
			self.sub.axvline(x=prad,color='%s'%clr,linestyle='--',linewidth=1)
		#	np.savetxt('Halo%s_raddenZ12_dmo.out'%(hnum),np.column_stack((dmox,dmomass/(4*3.14159*dmox**3)*0.83120300751/dlogr,dmonum)))
		elif res == '_14':
			self.sub.axvline(x=prad,color='%s'%clr,linestyle='--',linewidth=1)
			np.savetxt('Halo%s_raddenZ14_dmo.out'%(hnum),np.column_stack((dmox,dmomass/(4*3.14159*dmox**3)*0.83120300751/dlogr,dmonum)))
		elif res == '_13' or res == '_11':
			self.sub.axvline(x=prad,color='%s'%clr,linestyle='--',linewidth=1)
  ...
  #===========================================================================#
		...
		(fit, cmatrix)= opt.curve_fit(nfw,dmox[dmox>prad],np.log(dmoden[dmox>prad]),bounds = param_bounds)#dmox,dmomass/(4*3.14159*dmox**3),p0=(mvir,c))
		print fit
		if res == '_14':
	...
	def save(self,date,hnum,grain):
		...
		self.fig.savefig('radden_halo%s_dmotest_%s.pdf'%(hnum,date),transparent=True)#radden_halo%s_%dbin_%s.pdf'%(hnum,grain,date),transparent=True)
		self.fig.show()
		#plt.close()
...
def main():
  ...
  sm3._A = []
  hnum =['948','32503']#,'12596','007','848','796','20192','20910','897','948','1016','32257','10q','10v']
  ...
  dmover = ['11_13','11_13','11_13','11_13','11_13','11_13','5_12_16','5_12_16','11_13','11_13','11_13','11_13','5_12_16','5_12_16']
  ...
  for w in np.arange(len(hnum)):
   #for i in sp:
    ...
    try:
     ...
     dmomass,dmox,dmocNFW,dmovmax,dlogr,dmo500pc,dmonum,prad = radpro_df(pathname,hnum[w],res[w],dmover[w],hist,1,0,0,i,grain,dmover[w])
   
     mvir,c = radpro_plot.add_dmoline(dmox,dmomass,dlogr,res[w],dmocNFW,hnum[w],prad,dmonum)
     vcirc_plot.add_line(hnum[w],dmox,dmomass,res[w])
    # if hnum[w] == '2':
    #   hnum[w] = '848'
...
#       res[w] = '_11'
     ...
     #!hismass = smass
     #!hirhalf = rhalf
     #!radpro_plot.add_line(x,rhalf,totmass,dlogr,res[w],red,hnum[w],mvir,c,prad)
     ##if hnum == '948'
     ## radpro_plot = radpro(sm)

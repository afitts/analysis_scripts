...
class radpro(object):
  ...
  #===========================================================================#
		...
		(fit, cmatrix)= opt.curve_fit(einasto,dmox[dmox>prad],np.log(dmoden[dmox>prad]),bounds = param_bounds)#dmox,dmomass/(4*3.14159*dmox**3),p0=(mvir,c))
		print 'fit is ',fit
		#!self.sub.loglog(dmox,np.exp(nfw(dmox, *fit)),'g-',linewidth=2, label='nfw') #Core Fitting
		#self.sub.loglog(dmox,np.exp(einasto(dmox, *fit)),'g-',linewidth=2, label=r'einasto $\alpha=0.17$') #Core Fitting

...
#rcParams['savefig.pad_inches'] = .25
rcParams['font.family']='Bitstream Vera Serif'
...
def coords_checker():
	...
	for i in np.arange(len(hnum)):
		print hnum[i]
		hdf = pd.HDFStore('/nobackup/afitts/Gadget-2.0.7/production/mfm%s%s_giz%s_raw_output/analysis/dataframes/halo%s%s_giz%s_snap%03d.h5'%(hnum[i],res[i],ver[i],hnum[i],res[i],ver[i],snap[i]))
		x = np.float(hdf['props']['halox'])
		y = np.float(hdf['props']['haloy'])
		z = np.float(hdf['props']['haloz'])
		if hnum[i] != '10q' and hnum[i] != '10v' and hnum[i] != '1084':
			hdf = pd.HDFStore('/nobackup/afitts/Gadget-2.0.7/production/mfm%s%s_giz%s_raw_output/analysis/dataframes/halo%s%s_giz%s_snap%03d.h5'%(hnum[i],lowres[i],ver[i],hnum[i],lowres[i],ver[i],snap[i]))
			lowx = np.float(hdf['props']['halox'])
			lowy = np.float(hdf['props']['haloy'])
			lowz = np.float(hdf['props']['haloz'])
		if hnum[i] == '848' or hnum[i] == '948' or hnum[i] == '32503':
			hdf = pd.HDFStore('/nobackup/afitts/Gadget-2.0.7/production/gizdm%s%s_raw_output/analysis/dataframes/halo%s%s_giz%s_snap%03d.h5'%(hnum[i],'_14',hnum[i],'_14',ver[i],snap[i]))
			dmohix = np.float(hdf['props']['halox'])
			dmohiy = np.float(hdf['props']['haloy'])
			dmohiz = np.float(hdf['props']['haloz'])
		if hnum[i] == '848':
			hnum[i] = '2'
		hdf = pd.HDFStore('/nobackup/afitts/Gadget-2.0.7/production/gizdm%s%s_raw_output/analysis/dataframes/halo%s%s_giz%s_snap%03d.h5'%(hnum[i],res[i],hnum[i],res[i],dmover[i],snap[i]))
		dmox = np.float(hdf['props']['halox'])
		dmoy = np.float(hdf['props']['haloy'])
		dmoz = np.float(hdf['props']['haloz'])
		if hnum[i] != '10q' and hnum[i] != '10v' and hnum[i] != '1084':
			hdf = pd.HDFStore('/nobackup/afitts/Gadget-2.0.7/production/gizdm%s%s_raw_output/analysis/dataframes/halo%s%s_giz%s_snap%03d.h5'%(hnum[i],lowres[i],hnum[i],lowres[i],ver[i],snap[i]))
			dmolowx = np.float(hdf['props']['halox'])
			dmolowy = np.float(hdf['props']['haloy'])
			dmolowz = np.float(hdf['props']['haloz'])
		if hnum[i] == '2' or hnum[i] == '948' or hnum[i] == '32503':
			print 'MFM 13 - DMO 14 = ',np.sqrt((x-dmohix)**2+(y-dmohiy)**2+(z-dmohiz)**2)
		if hnum[i] != '10q' and hnum[i] != '10v' and hnum[i] != '1084':
			print 'MFM 13 - MFM 12 = ',np.sqrt((x-lowx)**2+(y-lowy)**2+(z-lowz)**2)
		print 'MFM 13 - DMO 13 = ',np.sqrt((x-dmox)**2+(y-dmoy)**2+(z-dmoz)**2)
		if hnum[i] != '10q' and hnum[i] != '10v' and hnum[i] != '1084':
			print 'MFM 13 - DMO 12 = ',np.sqrt((x-dmolowx)**2+(y-dmolowy)**2+(z-dmolowz)**2)
	return True
...
class trip_radpro(object):
	...
	def __init__(self,sm):
		...
		self.trip2.set_xlabel('$\mathrm{Radius}$ $\mathrm{(kpc)}$',fontsize=12)
		self.trip1.set_ylabel(r'$\rho$ $(\mathrm{M_\odot/kpc^3})$',fontsize=12)
...
class dbl_radpro(object):
	""" 1 x 2 subplot figure of all radial density profiles. Meant to display the transition of cuspy to cored. """
	def __init__(self,sm):
		self.fig = plt.figure()
		self.sub = self.fig.add_subplot(111)
		self.sm = sm
		#Initialize triple horizontal radial density profiles
		self.fig, (self.trip1, self.trip2) = plt.subplots(1,2,sharex=True,sharey=True)
		#self.trip2.set_xlabel('$\mathrm{Radius}$ $\mathrm{(kpc)}$',fontsize=12)
		self.fig.text(0.5, .2, '$\mathrm{Radius}$ $\mathrm{(kpc)}$',fontsize=16, ha='center')
		self.trip1.set_ylabel(r'$\rho$ $(\mathrm{M_\odot/kpc^3})$',fontsize=16)

	def add_line(self,hnum,res,ver,snap):
		for i in np.arange(len(hnum)):
			if hnum[i] == '848':
				dmox = np.genfromtxt('Halo%s_raddenZ13_dmo.out'%('2')) 
			else:
				dmox = np.genfromtxt('Halo%s_raddenZ13_dmo.out'%(hnum[i])) 
			dmomass = dmox[:,1]
			dmox = dmox[:,0]
			hydro = np.genfromtxt('Halo%s_raddenZ13_hydro.out'%(hnum[i]))
			totmass = hydro[:,1]
			x = hydro[:,0]
			pathname = '/nobackup/afitts/Gadget-2.0.7/production/mfm%s%s_giz%s_raw_output/analysis/'%(hnum[i],res[i],ver[i])
			hdf = pd.HDFStore('%sdataframes/halo%s%s_giz%s_snap%03d.h5'%(pathname,hnum[i],res[i],ver[i],snap[i]))
			smass = sum(hdf['particles/star']['mass'].as_matrix())
			if hnum[i] == '1084':
				self.trip2.loglog(x,totmass, color = 'k',linewidth=2,label = 'HYDRO')
				self.trip1.loglog(dmox,dmomass, color = 'k',linewidth=2, label = 'DMO')#'lightgrey'
			else:
				self.trip2.loglog(x,totmass, color = self.sm.to_rgba(np.log10(smass)),linewidth=2,label = 'HYDRO')
				self.trip1.loglog(dmox,dmomass, color = self.sm.to_rgba(np.log10(smass)),linewidth=2, label = 'DMO')#'lightgrey'
	...
	def save(self,date):
		#labels = ['%.5f' % float(t.get_text()) for t in ax.get_xticklabels()]
		self.fig.savefig('dbl_raddenpros_%s.pdf'%date,Transparent=True)
		self.fig.show()
		#plt.close()
...
class trip_correl(object):
	...
	def __init__(self,sm):
		...
		self.trip2.set_xlabel('$M_\star(<r_{1/2})$',fontsize=14)
		self.trip1.set_ylabel(r'$r_\mathrm{1/2}$ $(\mathrm{kpc})$',fontsize=14)
		self.trip2.set_ylabel(r'$M/L(<r_{1/2})$',fontsize=14)
		...
		self.trip1_yliml = np.log10(.1)#-.1#.15
		self.trip1_ylimh = np.log10(3)#2.2#1.3
		self.fig.sca(self.trip1)
		plt.tick_params(which='minor',labelsize=12,pad=2)
		ticks = np.linspace(-1,1,3)
		plt.yticks(ticks,['0.1','1','10'],fontsize=12)#[r'$10^1$',r'$10^2$'],fontsize=10) #[r'$10^6$',r'$10^7$',r'$10^8$',r'$10^9$'],fontsize=16)#
		minor_ticks=[]
		for j in range(2,10):
		  minor_ticks.append(-1+np.log10(j))
		for j in range(2,10):
		  minor_ticks.append(0+np.log10(j))
		self.trip1.yaxis.set_minor_locator(FixedLocator(minor_ticks))
		minor_labels = ['','0.3','','','','','','','','','','','','','','','','','','','','','','','','','',]#['','30 ','','','60 ','','','']
		self.trip1.yaxis.set_minor_formatter(FixedFormatter(minor_labels))
		self.trip1.set_ylim(self.trip1_yliml,self.trip1_ylimh)
		self.trip2_xliml = np.log10(5e4)
		...
		plt.yticks(ticks,['1','10','100','1000'],fontsize=12)#[r'$10^1$',r'$10^2$'],fontsize=10) #[r'$10^6$',r'$10^7$',r'$10^8$',r'$10^9$'],fontsize=16)#
		minor_ticks=[]
		for j in range(2,10):
		  minor_ticks.append(0+np.log10(j))
		for j in range(2,10):
		...
		plt.xticks(ticks,['$10^5$','$10^6$','$10^7$','$10^8$'],fontsize=12)#[r'$10^1$',r'$10^2$'],fontsize=10)
		minor_ticks=[]
		for j in range(2,10):
	...
	def add_point(self,rhalf,totrhalf,smrhalf,smass,res):
		if res == '':
			mkr = '^'
			self.trip1.plot([np.log10(sum(smass)),np.log10(sum(hismass))],[rhalf,hirhalf],'k')

		self.trip1.scatter(np.log10(smass),np.log10(rhalf),marker = 's',s=60,color='b')
		self.trip2.scatter(np.log10(smass),np.log10(totrhalf/smrhalf),marker = 's',s=60,color='b')
		...
		self.trip1.errorbar(isodwarf[:,6],np.log10(4/3.*isodwarf[:,0]/1000),yerr=[4/3.*isodwarf[:,2]/1000,4/3.*isodwarf[:,1]/1000],xerr=[isodwarf[:,8],isodwarf[:,7]],color='k', fmt='o',markersize=5,elinewidth=2)
		self.trip1.errorbar(satmwdwarf[:,6],np.log10(4/3.*satmwdwarf[:,0]/1000),yerr=[4/3.*satmwdwarf[:,2]/1000,4/3.*satmwdwarf[:,1]/1000],xerr=[satmwdwarf[:,8],satmwdwarf[:,7]],color='0.6', fmt='o',markersize=5,elinewidth=2)
		...
		self.trip2.errorbar(isodwarf[:,6],np.log10(isodwarf[:,12]),yerr=[np.log10((isodwarf[:,12]+isodwarf[:,14])/isodwarf[:,12]),np.log10(isodwarf[:,12]/(isodwarf[:,12]-isodwarf[:,13]))],xerr=[isodwarf[:,8],isodwarf[:,7]],color='k', fmt='o',markersize=5,elinewidth=2)
		...
		self.trip2.errorbar(satmwdwarf[:,6],np.log10(satmwdwarf[:,12]),yerr=[np.log10((satmwdwarf[:,12]+satmwdwarf[:,14])/satmwdwarf[:,12]),np.log10(satmwdwarf[:,12]/(satmwdwarf[:,12]-satmwdwarf[:,13]))],xerr=[satmwdwarf[:,8],satmwdwarf[:,7]],color='0.6', fmt='o',markersize=5,elinewidth=2)
		## vv for mdyn vv ##
...
class rhoratio(object):
	...
	def __init__(self,sm,ver):
		...
		self.ver = ver
		if ver == 0:
			self.sub.set_xlabel(r'$M_\star$ $(\mathrm{M}_\odot)$',fontsize=22)
			self.sub.set_xscale('log')
			self.sub.set_xlim(7e4,2e7)
			self.sub.set_ylabel(r'$\mathrm{\rho}_{\mathrm{dm(hydro)}}/\mathrm{\rho}_{\mathrm{dmo(corrected)}}\mathrm{(500pc)}$',fontsize=22)#"$v_{circ}$ (km/s))
			#plt.tight_layout()
			#self.sub.tick_params(axis='x', which='major', pad=5)
			#self.sub.xaxis.set_label_coords(.48,-.1)
			#plt.gcf().subplots_adjust(bottom=0.15)
		elif ver == 1:
			self.sub.set_xlabel(r'$r_{\mathrm{1/2}}$ $\mathrm{(kpc)}$',fontsize=22)
			self.sub.set_xlim(1e-1,1.3e0)
			self.sub.set_ylabel(r'$\mathrm{\rho}_{\mathrm{dm(hydro)}}/\mathrm{\rho}_{\mathrm{dmo(corrected)}}\mathrm{(500pc)}$',fontsize=22)#"$v_{circ}$ (km/s))
			self.sub.xaxis.set_label_coords(.48,-.06)
		elif ver == 2:
			self.sub.set_xlabel(r'$\mathrm{GoodnessOfFit}$',fontsize=22)
			self.sub.set_xlim(5e-3,1.3e-1)
			self.sub.set_ylabel(r'$\mathrm{\rho}_{\mathrm{dm(hydro)}}/\mathrm{\rho}_{\mathrm{dmo(corrected)}}\mathrm{(500pc)}$',fontsize=22)#"$v_{circ}$ (km/s))
		elif ver == 3:
			self.sub.set_ylabel(r'$\mathrm{GoodnessOfFit}$',fontsize=20)
			self.sub.set_ylim(5e-3,1.3e-1)
			self.sub.set_xlabel(r'$M_\star$',fontsize=20)#"$v_{circ}$ (km/s))
		elif ver == 4:
			self.sub.set_xlabel(r'$a_{50\%}$',fontsize=20)
			self.sub.set_ylim(5e-3,1.3e-1)
			self.sub.set_ylabel(r'$\mathrm{\rho}_{\mathrm{dm(hydro)}}/\mathrm{\rho}_{\mathrm{dmo(corrected)}}\mathrm{(500pc)}$',fontsize=16)#"$v_{circ}$ (km/s))
		#self.sub.legend(loc=2,prop={'size':10})
		#self.sub.set_yscale('log')
		#self.sub.set_ylim(.5,1.15)

		cb = self.fig.colorbar(self.sm)
		cb.set_label(r'$\mathrm{log}$ $M_\star$ ($z=0$)',fontsize=20)

	def add_point(self,rhoratios,smass,rhalf,gof,res,a50=0):
		if res == '':
			mkr = '^'
		else:
			mkr = 'o'
		...
		if self.ver == 0:
			self.sub.scatter(smass,rhoratios,marker = '%s'%mkr,s=80,color=self.sm.to_rgba(np.log10(smass)))

		elif self.ver == 1:
			self.sub.scatter(rhalf,rhoratios,marker = '%s'%mkr,s=80,color=self.sm.to_rgba(np.log10(smass)))

		elif self.ver == 2:
			self.sub.scatter(gof,rhoratios,marker = '%s'%mkr,s=80,color=self.sm.to_rgba(np.log10(smass)))
		elif self.ver == 3:
			self.sub.scatter(np.log10(smass),gof,marker = '%s'%mkr,s=80,color=self.sm.to_rgba(np.log10(smass)))
		elif self.ver == 4:
			a50 = np.array([0.241379310345,0.309865900383,0.241379310345,0.288793103448,0.836685823755,0.636494252874,0.357279693487,0.267720306513,0.73132183908,0.209770114943,0.193965517241,0.246647509579,0.557471264368,0.251915708812,0])
			self.sub.scatter(a50,rhoratios,marker = '%s'%mkr,s=80,color=self.sm.to_rgba(np.log10(smass)))
	...
	def save(self,date):
		if self.ver == 0:
		  ...
		  self.fig.savefig('Mstar_vs_GoF_loglog_%s.pdf'%date,transparent=True)
		elif self.ver == 4:
		  self.fig.savefig('rhohydrorhodmo_500pc_vs_a50_loglog_%s.pdf'%date,transparent=True)
		self.fig.show()
		#plt.close()
...
class radpro(object):
	...
	def add_line(self,x,rhalf,totmass,dlogr,res,red,hnum,mvir,c,prad,num):
		if res == '':
			...
			clr = 'k'
			lbl = 'HI '
			np.savetxt('Halo%s_raddenZ13_hydro.out'%(hnum),np.column_stack((x,totmass/(4*3.14159*x**3)/dlogr,(num))))
...
class mstarvmax(object):
	...
	def __init__(self):
		...
		self.sub.set_xscale('log')
		self.sub.set_xlabel(r'$V_\mathrm{max}$ $\mathrm{(km }$ $\mathrm{s^{-1})}$')
		self.sub.set_ylim(7e4,1e11)
		self.sub.set_yscale('log')
		self.sub.set_ylabel(r'$M_\star$ $(\mathrm{M}_\odot)$')
	...
	def add_point(self,vmax,mstar):
		print vmax,mstar
		self.sub.scatter(vmax,mstar,c='k',s=80, facecolor = 'red')
...
def main():
  ...
  #rhoratiovgof_plot= rhoratio(sm,3)
  dbl_radpro_plot = dbl_radpro(sm)
  ...
  #f = np.array([],dtype = [('name','|S10'),('Mdyn','>f4'),('Mgas','>f4'),('Mstar','>f4'),('Mdyn_rhalf','>f4'),('Mgas_rhalf','>f4'),('Mstar_rhalf','>f4'),('rhalf','>f4'),('vmax','>f4'),('Mvir_dmo','>f4'),('vmax_dmo','>f4'),('rhohydro_rhodmo_500pc','>f4'),('GoF','>f4')])
  bob = np.genfromtxt('plottbl.out')
  mvir = bob[:,1]
  mgas = bob[:,2]
  mstar = bob[:,4]
  totrhalf = bob[:,5]
  ...
  gof = bob[:,14] 
  coords_checker()
  #mstarvmax_plot.add_point(vmax,mstar)
  #trip_correl_plot.add_point(rhalf,totrhalf,smrhalf,mstar,res)
  #rhoratiovgof_plot.add_point(rhoratios,mstar,rhalf,gof,res)
  #rhoratiovrhalf_plot.add_point(rhoratios[:14],mstar[:14],rhalf[:14],gof[:14],res)
  #rhoratiovmstar_plot.add_point(rhoratios[:14],mstar[:14],rhalf[:14],gof[:14],res)
  #dbl_radpro_plot.add_line(hnum,res,ver,snap)
  #plt.show()
